...
```
                                   ...
                                   │           : 8080               │
                                   └──────────────┬──────────────┘
                            registers/discovers   │  routes by path predicate
                    ┌──────────────────────────────┼───────────────────────────────────┐
                    │                               │                                    │
          ┌─────────▼─────────┐          ┌─────────▼─────────┐              ┌──────────▼──────────┐
          │  Discovery Server  │◄─────────┤   User Service     │◄──Feign────►│  Inventory Service   │
          │  (Netflix Eureka)  │  register │      : 8081        │             │        : 8082        │
          │       : 8761       │           │ users · dealers ·  │             │ stock · cylinder      │
          └─────────▲──────────┘           │  auth (JWT)        │             │ types · pricing       │
                    │  all services         └─────────┬──────────┘             └───────────┬──────────┘
                    │  register here                   │                                    │
                    │                                  │            Kafka events             │
          ┌─────────┴─────────┐             ┌──────────▼───────────────────────────────────▼─────────┐
          │  Allocation Service│◄───────────►│                    Kafka Broker (+ Zookeeper)            │
          │       : 8083       │  publish/    │  user.registered · dealer.registered                     │
          └─────────┬──────────┘  consume     │  allocation.requested/approved/rejected/confirmed        │
                    │                          │  queue.joined/ready/completed/cancelled                  │
          ┌─────────▼──────────┐               └──────────────┬───────────────────────┬──────────────────┘
          │   Queue Service     │◄──────────────────────────────┘                       │
          │       : 8084        │  publish/consume                                       │
          └─────────┬───────────┘                                            ┌───────────▼───────────┐
